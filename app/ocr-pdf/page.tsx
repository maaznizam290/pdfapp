...
export default function OCRPDFPage() {
  ...
  const handleOCR = async () => {
    if (!file) {
      alert('Please select a PDF file to process with OCR.');
      return;
    }
    ...
    try {
      console.log('Starting OCR process with file:', { name: file.name, size: file.size });
      
      const options = {
        language: ocrOptions.language,
        outputFormat: ocrOptions.outputFormat,
        imageQuality: ocrOptions.imageQuality,
        preserveLayout: ocrOptions.preserveLayout
      };
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || errorData.details || 'Failed to process OCR');
      }

      const blob = await response.blob();
      
      console.log('OCR successful, blob size:', blob.size);
      ...
      alert(`Failed to process OCR: ${error.message || 'Unknown error'}`);
    } finally {
